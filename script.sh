...
#reads list of countries targeted by one APT group and write some stats for 'Region', into /tmp/project/ and 'Regime', into /tmp/regime  fields.
proportion() {
...
if [[ ! -z $(cat /tmp/$i) ]]
	then
		...
		if [ -f $HOME/Documents/misp/countries/$c.txt ]
			then
				grep -e "Region" $HOME/Documents/misp/countries/$c.txt | sed -E "s/ /_/g" | awk '{print $2}' | sed s/\"//g >> /tmp/project/$apt.txt
				grep -e "Regime" $HOME/Documents/misp/countries/$c.txt | sed -E "s/ /_/g" | awk '{print $2}' | sed s/\"//g >> /tmp/regimes/$apt.txt
			fi;done

	if [ -e /tmp/project/$apt.txt ]
		then
			cat /tmp/project/$apt.txt | awk '{a[$0]++}END{for(k in a){print k,a[k]}}' | sort -k2,2nr | sponge /tmp/project/$apt.txt
